...
| 第三栏 | 380px，高度 >= 600px | AI 旅行规划 iframe |
...
- 整体风格必须像小红书图文日志：轻松、真实、适合收藏，像游客当天边走边拍边记录，而不是官方攻略或机械清单
- 叙事视角使用“游览者/旅行者”第一视角，围绕当天路线写成旅行日志，包含到达、路上、拍照、吃饭、休息、回酒店等体验节点
- 内容必须包含天气提醒、乘车/打车/步行建议、餐饮建议、景点游览建议、打卡拍摄机位、女生着装建议和随身物品建议
- 酒店相关内容必须联网搜索补充到 `sidebar.html`：包括酒店位置、周边交通、周边餐饮/便利店、到主要景点的移动建议、入住/休息节奏建议；如果网络信息不足，必须明确写“信息不足，仅根据日程推断”
- 餐饮、景点、交通建议要尽量结合日程里的时间、地点和顺序，不要生成与当天路线明显冲突的内容
- 图文日志可以使用分段标题、贴纸感标签、emoji、短句和卡片式段落，但不要依赖外链图片；如需要图片氛围，用文字描述“拍什么、怎么拍、从哪个角度拍”
- 格式必须适配第三栏：宽度约 380px，高度强制 >= 600px，内容可纵向滚动，避免横向滚动、超宽表格和大段密集文字
- 每 60 分钟自动刷新；只有 `schedules.csv` 发生变化或联网补充信息需要更新时才重写 `sidebar.html`

**sidebar.html 内容准则**:
- 开头给出当天旅行主题和一句像小红书封面的短标题，例如“南京钟山线慢游日记”“不赶路版玄武湖散步”
- 每个日程段落写成“时间 + 地点/事项 + 游览者视角体验 + 实用建议”
- 天气模块要说明体感、雨具/防晒/外套需求，以及对拍摄和路线节奏的影响
- 交通模块要给出优先交通方式、预计换乘/步行提醒、容易踩坑的点，以及赶飞机/赶车时的保守出发建议
- 打卡拍摄模块要给出适合拍摄的时间段、构图建议、动作建议、避开人流建议；语气像给朋友做拍照攻略
- 女生着装模块要结合天气、步行强度、拍照地点和交通方式，给出颜色、鞋子、外套、包、补妆/防晒等建议
- 酒店模块必须通过网络搜索补全真实信息；不要凭空编造酒店设施、价格、评分或营业时间
- 结尾给出“今日小结”和“明天/下一段行程提醒”，帮助第三栏作为持续旅行日志使用
